...
- **STRICT — when CLAUDE does any testing, NEVER call a real/paid API. Mocks only.** This applies to Claude's own testing anywhere (local OR prod) and every provider: Mobile→PAN (Perfios prefill), MSG91 SMS, DigiLocker, iAdore, NuralX, Razorpay, and the LLM/gpt-4o gateway, plus any future vendor. Real vendor calls burn paid credits — a live financial issue. Run pytest / offline fixtures (they already patch the seams: offline judge, no vendor routing); never drive the browser journey or curl endpoints against configured creds. If a real call seems genuinely necessary, STOP and ask the user first — never call on your own judgment. (The USER testing on local or prod still calls real APIs by choice — that is fine; this rule only governs Claude's own testing.)
